    ...
    <div className="rounded-lg border border-border bg-muted/30 p-1 grid grid-cols-3 gap-1.5">
      {PROXY_TIERS.map((t) => (
        <button
          ...
          type="button"
          onClick={() => onChange(t)}
          className={`py-2.5 px-3 text-sm rounded-md transition-colors capitalize ${value === t ? 'bg-background shadow-sm font-medium' : 'text-muted-foreground hover:text-foreground'}`}
        >
          {t}
        </button>
      ))}
